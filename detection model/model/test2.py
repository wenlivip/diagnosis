...
def test_net(sess, net, imdb, weights_filename, max_per_image=100, thresh=0.):
  ...
  all_boxes = [[[] for _ in range(num_imgs)] for _ in range(12)]

  results = {'results': [], 'features': {}}

  for i in range(num_imgs):
    print('{}/{}'.format(i+1, num_imgs))
    img_pth = os.path.join(img_pre_pth, img_list[i])
      ...
      if len(image_scores) > max_per_image:
        ...
        for j in range(1, imdb.num_classes):
          ...
          if len(keep) == 1:
            ...
            results['results'].append(pred_str)
            results['features'][img_name] = roi_logits[feature_index, :]

  print('Done!')
  with open(os.path.join(pre_pth, 'results.pkl'), 'w') as f:
    pickle.dump(results, f)

  with open(os.path.join(pre_pth, 'prediction.csv'), 'w') as f:
    f.writelines(results['results'])
